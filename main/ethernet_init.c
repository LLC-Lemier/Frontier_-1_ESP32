...
esp_err_t esp_netif_receive_eapol(esp_eth_handle_t hdl, uint8_t *buffer, uint32_t length, void *priv)
{
    if (length > 14) {
        ...
        // Учтём и VLAN-tagged кадры: 802.1Q header сдвигает реальный EtherType.
        if (ethertype == 0x8100 && length > 18) {
            ethertype = (buffer[16] << 8) | buffer[17];
        }

        if (ethertype == 0x888E) {
            ESP_LOGD(TAG, "Received EAPoL frame, length: %" PRIu32, length);
    ...
    // Обычные кадры в лог не печатаем, чтобы не засорять monitor.
